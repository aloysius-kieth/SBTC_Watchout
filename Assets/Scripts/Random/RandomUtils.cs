...
namespace com.GMM.RandomUtils
{
	...
	public static class RandomUtils
	{
		...
		/// </summary>
		public static int RandomWeight(ObjectWeight[] weights)
		{
			#region Assert
			if (weights == null)
			{
			...
			int index = 0;
			while (r >= weights[index].weight)
			{
				r -= weights[index].weight;
				++index;
			}
			return index;
		}
		...
		/// Generate a Random Index of an Array, based on Weights
		/// </summary>
		public static int RandomWeight(ObjectWeight[] weights, int sum)
		{
			#region Assert
			if (weights == null)
			{
